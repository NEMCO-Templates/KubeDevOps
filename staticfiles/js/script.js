...
const dropdownMenu2 = document.querySelector('.notification-dropdown');

dropdownButton2.addEventListener('click', () => {
  dropdownMenu2.classList.toggle('open');
});

// Add click event listeners to links in dropdown menu
const links = dropdownMenu2.querySelectorAll('a');
links.forEach(link => {
  link.addEventListener('click', (event) => {
    event.stopPropagation(); // Stop event from bubbling up to button
  });
});
